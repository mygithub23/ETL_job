    ...
    States = {
      ...
      UpdateStatusCompleted = {
        ...
        Parameters = {
          ...
          ExpressionAttributeValues = {
            ...
            ":job_run_id"     = { "S.$" = "$.glue_result.JobRunId" }
          }
        }
      ...
      SendFailureAlert = {
        ...
        Parameters = {
          ...
          Subject  = "NDJSON Pipeline Failed - ${var.environment}"
          Message = {
            "FunctionError.$"  = "States.Format('Glue job failed for manifest: {}', $.manifest_path)"
            "DatePrefix.$"     = "$.date_prefix"
            "ManifestPath.$"   = "$.manifest_path"
            "Error.$"          = "$.error"
            "Timestamp.$"      = "$$.State.EnteredTime"
            "ExecutionArn.$"   = "$$.Execution.Id"
            "StateMachineName" = "ndjson-parquet-processor-${var.environment}"
          }
        }
        End = true
      }
