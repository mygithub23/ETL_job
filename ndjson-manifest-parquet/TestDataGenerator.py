...
# Environment Variables ndjson-input-sqs-804450520964
# INPUT_BUCKET = os.environ['INPUT_BUCKET']
# s3://ndjson-input-sqs-804450520964-dev/pipeline/input/
...
def lambda_handler(event, context):
    ...
    3. Generate files for specific date:
       {"file_count": 50, "date_prefix": "2025-12-26"}
    
    4. Generate custom size files:
       {"file_count": 5, "target_size_mb": 5.0}
    """
    date_prefix_list = ["2025-12-25", "2025-12-21", "2025-12-22"]
    processed_count = 0
    # result = {}
    try:
        for date_prefix in date_prefix_list:  
            print(f"Starting test file generation for date: {date_prefix}")
            print(f"Processed count: {processed_count}")
            # Initialize generator and uploader
            target_size_bytes = int(TARGET_FILE_SIZE_MB * BYTES_PER_MB)
            ...
            file_count = 20
            # date_prefix = event.get('date_prefix', DATE_PREFIX)  # None = today
